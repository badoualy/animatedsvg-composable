...
object SvgHelper {
     ...
     */
    fun extractPathData(input: String): List<String> = svgPathPattern.collect(input, 1)
    ...
    fun buildPath(pathData: String): Path {
        ...
        try {
            ...
            while (matcher.find()) {
                val command = matcher.group(1)?.get(0) ?: Char.MIN_VALUE
    ...
    private fun Pattern.collect(
        ...
        while (matcher.find()) {
            matcher.group(groupIndex)?.takeIf { it.isNotBlank() }?.let(list::add)
        }
